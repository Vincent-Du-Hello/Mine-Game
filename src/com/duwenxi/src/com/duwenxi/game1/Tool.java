...
public class Tool {
	public static void newBoard(ArrayList<ArrayList<GridType>> gameBoard) {
		...
		for(int i = 0; i < Config.WIDTH; i++) {
			for(int j = 0; j < Config.HEIGHT; j++) {
				if(new Random().nextInt() % (Config.WIDTH * Config.HEIGHT) <= Config.BOOMCOUNT) {
	...
	@SuppressWarnings("deprecation")
	public static void changeTexture(MainCanvas c) {
		Frame f = new Frame();
		...
		try {
			...
			Config.CLICKSND = Applet.newAudioClip(new URL("./config/com/duwenxi/sound/click.wav"));
			Config.BOOMSND = Applet.newAudioClip(new URL("./config/com/duwenxi/sound/boom.wav"));
			Config.WINSND = Applet.newAudioClip(new URL("./config/com/duwenxi/sound/win.wav"));
		} catch (Exception e) {
			StaticLog.log(Level.ERROR, "Exception!");
			e.printStackTrace();
		}
